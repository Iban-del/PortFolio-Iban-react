import { useState } from "react";
import type { LinkInterface } from "./Link";
import Link from "./Link";
import { motion } from "motion/react"

export interface NavBarInterface{
    buttons:Array<LinkInterface>

}

/** NavBar */
const NavBar = ({
    buttons
}:NavBarInterface) =>{

    const [showButton,setShowButton] = useState(false);


    const onHoverStart = () => {
        setShowButton(true)
    }
    
    const onHoverEnd = () => {
        setShowButton(false)
    }
    


    return <div className="w-[100%] transparent h-24">
        <div className="flex w-[100%] transparent">
            <motion.div
                onHoverEnd={()=>onHoverEnd()}
                className="m-9 w-[100%] flex space-x-4 transparent"
            >
                <motion.svg 
                    onHoverStart={()=>onHoverStart()}
                    onTap={()=>setShowButton((e)=>!e)}
                    whileHover={{rotate:30}}
                    className="cursor-pointer relative transparent" 
                    width="50" 
                    height="50" 
                    viewBox="0 0 15 15" 
                    fill="none" 
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path d="M1.5 3C1.22386 3 1 3.22386 1 3.5C1 3.77614 1.22386 4 1.5 4H13.5C13.7761 4 14 3.77614 14 3.5C14 3.22386 13.7761 3 13.5 3H1.5ZM1 7.5C1 7.22386 1.22386 7 1.5 7H13.5C13.7761 7 14 7.22386 14 7.5C14 7.77614 13.7761 8 13.5 8H1.5C1.22386 8 1 7.77614 1 7.5ZM1 11.5C1 11.2239 1.22386 11 1.5 11H13.5C13.7761 11 14 11.2239 14 11.5C14 11.7761 13.7761 12 13.5 12H1.5C1.22386 12 1 11.7761 1 11.5Z" fill="currentColor" ></path>
                </motion.svg>

                {showButton &&
                <motion.div
                    initial={{left:0}}
                    whileInView={{left:20}}
                    className="relative flex space-x-4 transparent text-sm items-center justify-around"
                >
                    {buttons.map((e)=>{
                        return <Link key={e.key} onClick={e.onClick} text={e.text}></Link>
                    })}
                </motion.div>
                }

            </motion.div>
            
        </div>        
    </div>
}

export default NavBar;